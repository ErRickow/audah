...
async def module_help(client: Client, message: Message):
    ...
    if len(cmd) > 1:
        ...
        try:
            ...
            ac = PrettyTable()
            ac.header = False
            ac.title = "Anjeng-Ubot Plugins"
            ac.align = "l"
            for x in split_list(sorted(CMD_HELP.keys()), 2):
                ac.add_row([x[0], x[1] if len(x) >= 2 else None])
            xx = await client.send_message(
                message.chat.id,
                f"```{str(ac)}```\n• {client.me.mention} x {ubot.me.mention} •",
    ...
    if help_arg:
        if help_arg in CMD_HELP:
            ...
            this_command = f"<blockquote>──「 **Help For {str(help_arg).upper()}** 」──\n\n"
            for x in commands:
...
async def module_helper(client: Client, message: Message):
    ...
    if len(cmd) > 1:
        ...
        for x in split_list(sorted(CMD_HELP.keys()), 2):
            ac.add_row([x[0], x[1] if len(x) >= 2 else None])
        await message.reply(
            f"<blockquote>{str(ac)}\n• {client.me.mention} x {ubot.me.mention} •</blockquote>", parse_mode=enums.ParseMode.HTML
        )
