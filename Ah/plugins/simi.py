...
async def send_simtalk(message: str) -> str:
    if len(message) > 1000:
        ...
        try:
            ...
            if response.status_code != 200 or "message" not in result:
                raise ValueError("Invalid response from Simsimi API.")
...
async def chatbot_response(client, message: Message):
    ...
    try:
        ...
        logger.error(f"Error di chatbot_response: {str(e)}")

# Handler untuk mengatur status chatbot (on/off)
@Client.on_message(filters.command(["uy", "yu"], cmd) & filters.me)
async def manage_chatbot_status_dev(client, message: Message):
    ...
    try:
        ...
        logger.error(f"Error di manage_chatbot_status_dev: {str(e)}")
